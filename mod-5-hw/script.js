...
//   return arr.filter(elem => !elem.isActive);
// };
...
//   return arr.filter(elem => elem.email == email);
// };
...
//   return arr.reduce((acc,elem)=> acc + elem.balance ,0)
// }; 
...
//отфильтруем елементы массива, у которых ЕСТЬ указанный друг
//на основании отфильтрованного массива создадим новый массив и запишем туда только ИМЕНА тех, 
// у кого есть ОТФИЛЬТРОВАННЫЕ друзья
const getUsersByFriend = (arr, name) =>  arr.filter(elem => elem.friends.indexOf(name)>=0).map(x => x.name); 
...
const getAllUniqueSkills = arr =>  arr.map(elem => elem.skills) // собираем массив массивов с полем [warrior]  
.reduce((acc,elem) => acc.concat(elem),[]) // объединяем вложенные массивы в один
.filter((value, index, self) => self.indexOf(value) === index) // фильтруем одинковые елементы массива
...
const getUserNamesSortedByFriendsCount = arr => { 
  let arrNames = arr.sort((a,b)=> a.friends.length - b.friends.length).map(el => el.name);
  return arrNames;
} 
